...
<capitanes>{
      ...
      return <capitan equipo="{$equipo/@nombre/data()}">{
                             for $capi in /jugadores/jugador
                             where $capi/@cod=$equipo/capitan
                             return ($capi/nombre/data(),$capi/apellido1/data(), $capi/apellido2/data() )}
                             </capitan>}
 </capitanes>,
"-------------------------",
<capitanes>{
      for $equipo in /equipos/equipo, $capi in  /jugadores/jugador
      where $capi/@cod=$equipo/capitan
      return <capitan equipo="{$equipo/@nombre/data()}">{($capi/nombre/data(),$capi/apellido1/data(), $capi/apellido2/data() )}</capitan>
}</capitanes>,
...
return 
    concat($anyo,": ",count(//jugadores/jugador[year-from-date(fecha_nacimiento)=$anyo]))
,
